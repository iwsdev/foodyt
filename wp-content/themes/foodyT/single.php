...
$fb = new Facebook\Facebook([
  'app_id' => '1945628652377706', // Replace {app-id} with your app id
  'app_secret' => '3a7b47a6f10b0982515666c670087660',
  'default_graph_version' => 'v2.9',
  ]);
//$fb->destroySession();
 
$helper = $fb->getRedirectLoginHelper();
$logoutUrl = $helper->getLogoutUrl($facebook_access_token, $currPageUrl);
if(isset($info->data->user_login)){
}else{
	echo '<a id="fblogout" style="display:none;" href="' . $logoutUrl . '">Logout of Facebook!</a>';
}
...
else
{
   try
   {
      ...
      $response = $fb->get('/me?fields=id,picture,name,first_name,last_name,email,gender,birthday,age_range,hometown,locale,location,timezone,website,about', $accessToken);
    ...
    if(!empty($user))
    {
        ...
        if(empty($userexist ))
        {
            ...
            if($insertuser_id >0)
            {
                ...
                update_user_meta($insertuser_id, 'gender', $gender);
				        update_user_meta($insertuser_id, 'picture', $profile);
      ...
      var dishCountInfo = window.opener.document.getElementById(dishCount).value;
      for(var i=1;i<=dishCountInfo;i++){
	   var name = 'name' +i;
	   var email = 'email' +i;
	   var userid = 'userid' +i;
	   var comment_form = 'comment_form' + i;
	   var showWriteIcon = 'showWriteIcon' + i;
	   var box = 'box' + i;
	   var fbNoNeedLogin = 'fbNoNeedLogin' + i;
	   var fbNeedLogin = 'fbNeedLogin' + i;
       window.opener.document.getElementById(name).value ='".$first_name."';
       window.opener.document.getElementById(email).value ='".$email."';
       window.opener.document.getElementById(userid).value ='".$insertuser_id."';
       window.opener.document.getElementById(comment_form).style.display = 'block';
       window.opener.document.getElementById(showWriteIcon).style.display = 'block';
       window.opener.document.getElementById(fbNoNeedLogin).style.display = 'inline-block';
       window.opener.document.getElementById(box).style.display = 'none';
       window.opener.document.getElementById(fbNeedLogin).style.display = 'none';
       }
	  window.close();
	  </script>";
...
$catList = $wpdb->get_results("SELECT * from $menu_categorie_table where entry_by =" . $userId." ORDER BY cat_order");
  ...
  <header id="header" class="mobile">
    <div class="container">
      ...
      <div class="headerRight">
        <div class="headerContact">         
          <ul>
          	<li><a href="tel:954542446"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/phone.svg"></a></li>
            <li><a href="javascript:void(0)" data-toggle="modal" data-target="#g-map"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/map.svg"></a></li>            
          </ul>
        </div>        
      </div>
    </div>
...
<section id="mobileNav" class="mobile">
  <div class="container">
    <ul>
      ...
      <li id="grid" class="active"><a href="#" class="grid-view"> &nbsp;</a></li>
      <li id="list"><a href="#" class="list-view">&nbsp; </a></li>
    </ul>
  </div>
</section>
      ...
      ?>
        ...
        foreach ($menuDetails as $dishList) {?>
          <li>
            ...
            <a href="#" data-toggle="modal" data-target="#myModal<?= $j;?>" onclick="Viewdish('<?php echo $dishList->id;?>');"></a>
          </li>
          <?php $j++;} ?>
        </ul>
      </div>
    ...
    <div class="wrap">
      ...
      <div class="content">
        <h3><?php echo $dishList->name;?> </h3>
         <?php 
              if($dishList->price && $priceNotRequired==0){
              //echo '<span class="bullets"></span>';
              echo '<span style="padding: 0px 0px 0px 9px;" class="menu_price"><b>';
              echo  $dishList->price."&euro;" ?> </b></span>
              <?php } else{
              //echo '<span class="bullets"></span>';
                }?>
        <div class="foodlist">
...
<div id="myModal<?php echo $j; ?>" class="modal fade" role="dialog">
	<div class="close-btn"><button type="button" class="close" data-dismiss="modal">X</button></div>
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <?php 
      $urlPage2go2 =site_url('/')."restaurant/dish/".$slugName."/".$dishList->id.".html";
      $urlPage2go =$urlPage;
      $dish_id = $dishList->id;
    ...
    <div class="sharing">
      ...
      <div class="right">
        <ul>
          <li class="wp-icon">
              <a data-dishid="<?= $dish_id?>"  data-text="<?=$dishList->name?> " data-desc="  <?= $dishList->description; ?>" data-link="<?= $urlPage;?>" data-img="<?= $urlPage; ?>" class="whatsapp whatsCount w3_whatsapp_btn w3_whatsapp_btn_large"><img src="<?php echo bloginfo('template_url')?>/assets/images/wp-icon.png"></a>
         </li>
     ...
     <li> <a  data-dishid="<?= $dish_id?>" class="fbCount" href="javascript:void(0);" data-layout="button" onclick="window.open('https://www.facebook.com/sharer.php?u=<?= $urlPage2go2;?>&summary=MySummary&title=<?=$menuDetail->name?>&description=<?= $dishList->description; ?>&picture=<?= $dishList->attachment; ?>','ventanacompartir', 'toolbar=0, status=0, width=650, height=450');"> <img src="<?php echo bloginfo('template_url')?>/assets/images/fb-icon.png"></a></li>
      <?php }else{?>
      <li id="<?php echo "fbNeedLogin".$j; ?>" ><a href="#" onclick="window.open('<?php echo htmlspecialchars($loginUrl);?>','myWindow', 'width=550,height=400,left=400,top=200')" ><img src="<?php echo bloginfo('template_url')?>/assets/images/fb-icon.png"></a></li>
      <li id="<?php echo "fbNoNeedLogin".$j; ?>" style="display:none;"> <a  data-dishid="<?= $dish_id?>" class="fbCount" href="javascript:void(0);" data-layout="button" onclick="window.open('https://www.facebook.com/sharer.php?u=<?= $urlPage2go2;?>&summary=MySummary&title=<?=$menuDetail->name?>&description=<?= $dishList->description; ?>&picture=<?= $dishList->attachment; ?>','ventanacompartir', 'toolbar=0, status=0, width=650, height=450');"> <img src="<?php echo bloginfo('template_url')?>/assets/images/fb-icon.png"></a></li>
      <?php }?>
   ...
   <?php   $getReviewAll = $wpdb->get_results("SELECT * FROM $commentTable where dish_id = $dish_id AND comment_approved = 1");?>
    ...
    <div class='rating_overview'>
        ...
        <?php 
            ...
            {
            ?>
        <div class="write-icon" id="<?php echo "box".$j; ?>">
          <a href="javascript:void(0)"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/write-msg.png" alt=""/></a>
        </div>
        <?php }else{ ?>
      <div class="write-icon1" id="<?php echo "box".$j; ?>">
          <a href="javascript:void(0)"><img  onclick="window.open('<?php echo htmlspecialchars($loginUrl);?>','myWindow', 'width=550,height=400,left=400,top=200')" src="<?php echo get_template_directory_uri(); ?>/assets/images/write-msg.png" alt=""/></a>
        </div>
      <div class="write-icon" id="<?php echo "showWriteIcon".$j; ?>"  style="display:none;">
          <a href="javascript:void(0)"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/write-msg.png" alt=""/></a>
      </div>
          <?php  } ?>
        
        <form method="post" class="comment_form form" id="<?php echo "comment_form".$j; ?>">
          ...
          <div class="fooBar">
            ...
            <?php 
            if(!isset($info->data->user_login)){?>
            <input type="hidden"  id="<?php echo "name".$j; ?>" name="name">
            <input type="hidden" id="<?php echo "email".$j; ?>" name="email" value="<?=  $_SESSION['email']?>">
            <input type="hidden" id="<?php echo "userid".$j; ?>" name="userid" value="<?=  $_SESSION['insertuser_id']?>">
            <?php }else{?>
            <input type="hidden" name="name" value="<?=  $info->data->user_login?>">
            <input type="hidden" name="email" value="<?=  $info->data->user_email?>">
            <input type="hidden" name="userid" value="<?=  $info->data->ID?>">
              
           <?php } ?>
          
           <button id="subSend" type='submit' class="btn btn-primary submitbtn"><?= $_SESSION['lan']['rating_form']['send']?> </button>
         ...
         foreach ($getReviewAll as $data)
          {
			...
			$fb = get_user_meta($user_id,'registervia',true);
            ...
            {
               $imgUrl = get_user_meta($user_id,'picture',true);
...
<?php $j++;} }}?>
<!--  For count number of dish the dish value are used in javascript above after login facebook-->
<input type="hidden" value="<?= ($j-1)?>" id="dishCount">
