...
if($_REQUEST['restaurantName']){
	...
	$query = "SELECT  * FROM  $wppost WHERE post_type ='restaurant' AND post_status ='publish' AND post_title LIKE '%$searchName%'";
	...
	$querystr = "SELECT * FROM $wppost
	    ...
	    WHERE   $wppost.post_status = 'publish' AND $wppost.post_type = 'restaurant' AND v1.meta_value LIKE '%$searchName%' AND v1.meta_key = 'address' ORDER BY $wppost.post_date DESC";
	$res = $wpdb->get_results($querystr);
...
elseif($_REQUEST['cuisine']){
	...
	$querystr = "SELECT * FROM $wppost
	    LEFT JOIN $wppostmeta v1 ON ($wppost.ID = v1.post_id)
	    WHERE   $wppost.post_status = 'publish' AND $wppost.post_type = 'restaurant' AND v1.meta_value LIKE '%$searchName%' AND v1.meta_key = 'cusine'ORDER BY $wppost.post_date DESC";
	$res = $wpdb->get_results($querystr);
